...
try:
    while True:
        ...
        socket.send_string("REQUEST_SLOTS")  # 서버에 주차 데이터 요청
        try:
            parking_slots = socket.recv_string()  # 서버로부터 RFID 리스트 수신
            print(f"주차칸 Data 수신 완료: {parking_slots}")
            lcd.clear()
            lcd.write_string("SLOT REMAIN:")
            lcd.cursor_pos = (1, 0)  # 두 번째 줄
            lcd.write_string(f"{parking_slots} : 3")      
        except zmq.Again:
            print("서버 응답 지연 - 주차칸 Data를 받을 수 없습니다.")
